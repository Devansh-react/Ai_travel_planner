...
import { intentAnalyzer } from "../src/services/intentAnalyzer";
import { replyGenerator } from "../src/services/replyGenerator";
import { leadRepository } from "../src/repository/leadRepository";

const mockedExtractionService = extractionService as jest.Mocked<typeof extractionService>;
const mockedIntentAnalyzer = intentAnalyzer as jest.Mocked<typeof intentAnalyzer>;
const mockedReplyGenerator = replyGenerator as jest.Mocked<typeof replyGenerator>;
const mockedLeadRepository = leadRepository as jest.Mocked<typeof leadRepository>;
...
describe("sample conversation e2e", () => {
  ...
  test("walks through the assignment sample conversation", async () => {
    ...
    const turn1 = await conversationManager.handleMessage(
      "conv-sample",
      "I'm planning a honeymoon in Bali this December."
    );
    const turn2 = await conversationManager.handleMessage("conv-sample", "Two adults.");
    const turn3 = await conversationManager.handleMessage("conv-sample", "Around Rs 2 lakh.");
    const turn4 = await conversationManager.handleMessage(
      "conv-sample",
      "Rahul Verma, +91 99999999999"
    );
